...
public class ExportAction extends Action implements IWorkbenchAction,
	...
	public ExportAction(IWorkbenchWindow window) {
		...
		setEnabled(false);
		setId(ID);
		setText(Messages.ExportAction_Action_Title); 
		setToolTipText(Messages.ExportAction_Action_Tooltip); 
		...
		if (validator.validateSceneGraph()) {
			ExportProfileProvider provider = new ExportProfileProvider(
					Application.getCurrentProject().getSettings()
							.getResolutionWidth(), Application
							.getCurrentProject().getSettings()
							.getResolutionHeight());
			...
			try {
				...
				logger.error(String.format("Can't open the export wizard! %s", //$NON-NLS-1$
						e.getMessage()));
				return;
			}
			if (availableExportProfiles == null) {
				logger.warn("Can't open the export wizard, because getting the export profiles failed!"); //$NON-NLS-1$
				return;
			}
				...
				for (ExportProfile exportProfile : exportProfiles) {
					if (exportProfile.isChecked()) {
						...
						final ExportJob exportJob = new ExportJob( Application.getCurrentProject(),
								...
								exportProfile, outputFolder, currentExport,
								exportCount);
						try {
							...
							if (wizard.isOpenFolderSet()) {
								try {
									...
									MessageDialog messageDialog = new MessageDialog(
											...
											null,
											Messages.ExportAction_OpenFolderErrorMsg,
											MessageDialog.ERROR,
											new String[] { Messages.ExportAction_Button_Ok },
											0);
									messageDialog.open();
								}
							}
						...
						// no scheduling for background processing, the exportJobs run within the progresswindow.
						// exportJob.schedule();
					}
				}
			}
