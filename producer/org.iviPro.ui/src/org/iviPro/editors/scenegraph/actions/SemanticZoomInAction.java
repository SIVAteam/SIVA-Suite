...
/**
 * Action zum semantischen Zoom-In.
 * 
 * @author grillc
 * 
 */
public class SemanticZoomInAction extends SelectionAction {

	public static final String ID = SemanticZoomInAction.class.getName();
	...
	public SemanticZoomInAction(IWorkbenchPart part) {
		super(part);
		setId(ID);
		setText("Semantic Zoom In"); //$NON-NLS-1$
		setToolTipText(Messages.SemanticZoomInAction_SemanticZoomIn);
		setImageDescriptor(Icons.GRAPH_CONTEXTMENU_SEMANTICZOOMIN.getImageDescriptor());
	...
	public void run() {
		...
		try {
			...
			if(selectedZoomableObjects.size() != 0 && !ps.isFullSemanticZoomEnabled()) {
				//Ändere Zoomlevel
				for(int i=0; i < selectedZoomableObjects.size(); i++) {
					...
					if(currentNodeZoomLevel <= 0) {
						currentNodeZoomLevel = 1;
					}	
					newNodeZoomLevel = currentNodeZoomLevel + 1;
					zoomLvlChanged = true;

					if(newNodeZoomLevel > 3) {
						newNodeZoomLevel = 3;
						zoomLvlChanged = false;
					}
			...
			if(zoomLvlChanged) {
				this.firePropChange(LayoutManager.PROP_SEMANTIC_ZOOM_IN, null, editPartSizeChangedList);
			}
		} catch(Exception e) {
			e.printStackTrace();
		}
