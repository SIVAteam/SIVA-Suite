...
class ProjectCreateWizardPage extends WizardPage {
	...
	protected ProjectCreateWizardPage() {
		super(""); //$NON-NLS-1$
	...
	private Listener createInputChangeListener() {
		Listener listener = new Listener() {
			...
			public void handleEvent(Event event) {
				...
				String curProjectDirPath = curParentDirPath + File.separator
						+ curProjectName;
				File curParentDir = new File(curParentDirPath);
				File curProjectDir = new File(curProjectDirPath);
				...
				if (pageComplete) {
					data.projectDir = curProjectDir;
					data.projectName = curProjectName;
				}
