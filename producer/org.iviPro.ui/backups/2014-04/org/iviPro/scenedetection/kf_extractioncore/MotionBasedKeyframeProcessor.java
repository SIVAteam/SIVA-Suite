 ...
 */
public class MotionBasedKeyframeProcessor implements ControllerListener {
	 ...
	 */
	MotionBasedKeyframeProcessor(MotionBasedKeyframeSubWorker sub,
			int threadId, List<Shot> shotList, long[] threadFrames) {
		this.sub = sub;
	 ...
	 */
	public void runKeyFrameDetection() throws IllegalStateException {
		if (isRealized()) {
			open(ml, fileSize);
		}
	...
	// private void runImageExtraction(List<Long> frames)
	// throws IllegalStateException {
	// if (isRealized()) {
	// open(ml, fileSize);
	// }
	...
	// }
	//
	// addAlgorithms(false, frames);
	// realize();
	//
	// proc.setMediaTime(new Time(startFrame / frameRate));
	// proc.setStopTime(new Time(((endFrame) / frameRate)));
	// System.out.println("STARTTIME: "+proc.getMediaTime().getSeconds());
	// System.out.println("ENDTIME:" +proc.getStopTime().getSeconds());
	// start();
	// }
	...
	private void addAlgorithms(boolean start, List<Long> frames) {
		...
		try {
			...
			if (start) {
				if (AlgorithmSettings.getClassInstance().isKeyframePME()) {
					algorithms.add(new MotionBasedKeyframeAlgorithm(threadId,
							threadFrameNr, cuts));
				} else {
					algorithms.add(new SimpleKeyframeAlgo(threadId,
							threadFrameNr, cuts));
				}
			}
	...
	public void controllerUpdate(ControllerEvent evt) {
		if (evt instanceof ConfigureCompleteEvent
			...
			for (int i = 0; i < algorithms.size(); i++) {
				if (AlgorithmSettings.getClassInstance().isKeyframePME()) {
					if (algorithms.get(i) instanceof MotionBasedKeyframeAlgorithm) {
						if (!finalized) {
							MotionBasedKeyframeAlgorithm algo = (MotionBasedKeyframeAlgorithm) algorithms
									.get(i);
							frames = algo.finalizeAlgorithm();
					...
					if (algorithms.get(i) instanceof SimpleKeyframeAlgo) {
						if (!finalized) {
							SimpleKeyframeAlgo algo = (SimpleKeyframeAlgo) algorithms
									.get(i);
							frames = algo.finalizeAlgorithm();
							Collections.sort(frames);
							finalized = true;
			...
			//
			// if (finalized && !finished && !interrupt) {
			// open(ml, size);
			// runImageExtraction(frames);
			// finished = true;
			// } else {
			// if (finalized && finished && !interrupt) {
			// bugCounter++;
			// System.out
			// .println("JETZ MUSS I WIRKI SDCHAUN: BUGCOUNTERRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRR: "
			// + bugCounter);
			// if (bugCounter == 1) {
			// for (int i = 0; i < algorithms.size(); i++) {
			// if (algorithms.get(i) instanceof MotionBasedImageExtraction) {
			// MotionBasedImageExtraction algo = (MotionBasedImageExtraction)
			// algorithms
			// .get(i);
	...
	void setInterrupt() {
		System.out.println("JETZT GIBTS DEN INTERRUPT!!!!");
		this.interrupt = true;
	}
