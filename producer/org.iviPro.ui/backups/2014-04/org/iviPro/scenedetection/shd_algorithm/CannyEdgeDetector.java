 ...
 */
public class CannyEdgeDetector implements EdgeDetectionAlgorithm {
	 ...
	 * 
	 * @param src_1d
	 *            The source image as a pixel array
	 * @param width
	 *            width of the destination image in pixels
	 * @param height
	 *            height of the destination image in pixels
	 * @param size
	 *            The size of the kernel used in the smoothing
	 * @param theta
	 *            The gaussian smoothing standard deviation
	 * @param lowthresh
	 *            The low threshold for the tracking
	 * @param highthresh
	 *            The high threshold for the tracking
	 * @param scale
	 *            The amount of scaling to be applied to the image
	 * @param offset
	 *            The amount to be added to each result pixel
	...
	/*
	 * a) assume the image is grey level (hence RR=GG=BB) b) use value
	 * &0x000000ff to get the BB value c) gaussian smooth image d) work out
	 * gradient magnitude e) apply nonmaxima suppression f) threshold and detect
	 * edges
	 */
	@Override
	public int[] process(int[] src_1d, int width, int height, int size,
		...
		tmp_1d = GaussianSmooth
				.smooth_image(src_1d, width, height, size, theta);
		...
		for (int i = 0; i < d_w; i++) {
			for (int j = 0; j < d_h; j++) {
				...
				int arrayindex = (i + (j * (d_w + 3)));
				dest_1d[arrayindex] = 0xff000000 | result << 16 | result << 8
						| result;
				if (dest_1d[arrayindex] != -16777216) {
					edgeCounter++;
				}
	...
	private int suppress(int[][] m_2d, int sector, int i, int j, int lowthresh) {
		...
		// 117)System.out.println("ij("+
		// i+","+j+") sector: "+sector+" neigh: "+m_2d
		// [i-1][j-1]+" "+m_2d[i-1][j]
		// +" "+m_2d[i-1][j+1]+" "+m_2d[i][j-1]+" "+m_2d
		// [i][j]+" "+m_2d[i][j+1]+" "
		// +m_2d[i+1][j-1]+" "+m_2d[i+1][j]+" "+m_2d[i+1][j+1]);
	...
	/*
	 * The function apply_track is used to track the image for suitable lines.
	 * Itdoes this by first finding points above the highthreshold. When it
	 * findssuch a point it then finds surrounding point which are above the low
	 * thresholdand tracks along them, continually finding points above the low
	 * threshold. Thisis done until the tracker explores all paths from the
	 * original point. It thenfinds the next starting point and starts tracking
	 * again.
	 */
	private int[][] apply_track(int[][] input, int width, int height,
		...
		Stack<Point> to_track = new Stack<Point>();
	 ...
	 */
	private int[][] track(int[][] input, int[][] marked, Stack<Point> to_track,
		...
		while (!empty) {
			try {
				...
				current = to_track.pop();
				// Find it's co-ordinates
				a = current.x;
				b = current.y;
				// Now check neighbourhood and add to stack anything above
				// thresh
