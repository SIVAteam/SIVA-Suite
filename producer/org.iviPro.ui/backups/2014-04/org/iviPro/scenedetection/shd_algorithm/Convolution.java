...
public class Convolution extends Thread {
     ...
     *
     * @param input
     *            The 2D double array representing the image.
     * @param x
     *            The x coordinate for the position of the convolution.
     * @param y
     *            The y coordinate for the position of the convolution.
     * @param k
     *            The 2D array representing the kernel.
     * @param kernelWidth
     ...
     */
    public static double singlePixelConvolution(double[][] input, int x, int y,
            double[][] k, int kernelWidth, int kernelHeight) {
        double output = 0;
    ...
    public static int applyConvolution(int[][] input, int x, int y,
            double[][] k, int kernelWidth, int kernelHeight) {
        int output = 0;
        for (int i = 0; i < kernelWidth; ++i) {
            for (int j = 0; j < kernelHeight; ++j) {
     ...
     */
    public static double[][] convolution2D(double[][] input, int width,
            int height, double[][] kernel, int kernelWidth, int kernelHeight) {
     ...
     * over the area of the image specified by width and height and returns a
     ...
     * @return the 2D array representing the new image
     */
    public static double[][] convolution2DPadded(double[][] input, int width,
            int height, double[][] kernel, int kernelWidth, int kernelHeight) {
        int smallWidth = width - kernelWidth + 1;
        int smallHeight = height - kernelHeight + 1;
        ...
        small = convolution2D(input, width, height, kernel, kernelWidth,
                kernelHeight);
        ...
        for (int j = 0; j < smallHeight; ++j) {
            for (int i = 0; i < smallWidth; ++i) {
                // if (i+left==32 && j+top==100)
                // System.out.println("Convolve2DP: "+small[i][j]);
     ...
     */
    public static double[] convolutionDouble(double[][] input, int width,
            int height, double[][] kernel, int kernelWidth, int kernelHeight) {
        int smallWidth = width - kernelWidth + 1;
        int smallHeight = height - kernelHeight + 1;
        double[][] small = new double[smallWidth][smallHeight];
     ...
     * @return the 1D array representing the new image
     */
    public static double[] convolutionDoublePadded(double[][] input, int width,
            int height, double[][] kernel, int kernelWidth, int kernelHeight) {
        double[][] result2D = new double[width][height];
        result2D = convolution2DPadded(input, width, height, kernel,
                kernelWidth, kernelHeight);
        ...
        for (int j = 0; j < height; ++j) {
            for (int i = 0; i < width; ++i) {
                ...
                // if (i==32 && j==100)
                // System.out.println("ConvolveDP: "+result[j*width
                // +i]+" "+result2D[i][j]);
            }
        }
        return result;
     ...
     *            the height of the kernel
     * @param iterations
     *            the number of iterations to apply the convolution
     * @return the 2D array representing the new image
     */
    public double[][] convolutionType1(double[][] input, int width, int height,
            double[][] kernel, int kernelWidth, int kernelHeight, int iterations) {
        ...
        for (int i = 0; i < iterations; ++i) {
            ...
            output = convolution2D(newInput, width, height, kernel,
                    kernelWidth, kernelHeight);
            width = smallWidth;
            height = smallHeight;
     ...
     * @return the 2D array representing the new image
     */
    public double[][] convolutionType2(double[][] input, int width, int height,
            double[][] kernel, int kernelWidth, int kernelHeight, int iterations) {
        double[][] newInput = (double[][]) input.clone();
        double[][] output = (double[][]) input.clone();
        ...
        for (int i = 0; i < iterations; ++i) {
            ...
            output = convolution2DPadded(newInput, width, height, kernel,
                    kernelWidth, kernelHeight);
    ...
    public static int[] convolution_image(int[] input, int width, int height,
            double[][] kernel, int kernelWidth, int kernelHeight, double scale,
            double offset) {
        ...
        output = convolutionDoublePadded(input2D, width, height, kernel,
                kernelWidth, kernelHeight);
