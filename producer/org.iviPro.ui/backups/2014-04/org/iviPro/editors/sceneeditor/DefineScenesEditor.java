 ...
 * 
 */
public class DefineScenesEditor extends IAbstractEditor implements
		PropertyChangeListener {

	private static Logger logger = Logger.getLogger(DefineScenesEditor.class);
	public static final String ID = DefineScenesEditor.class.getName();
	...
	public void init(IEditorSite site, IEditorInput input)
		...
		if (getEditorInput() instanceof DefineScenesShotInsertEditorInput) {
			DefineScenesShotInsertEditorInput currentInput = (DefineScenesShotInsertEditorInput) input;
			video = currentInput.getVideo();
			
			mp = PlayerFactory.getPlayer(video, currentInput.getStart(), currentInput.getEnd());
		} else {
		...
		if (!(getEditorInput() instanceof DefineScenesShotInsertEditorInput)) {
			...
			for (int i = 0; i < tabs.length; i++) {
				SceneDefineWidget defineWidget = (SceneDefineWidget) tabs[i]
						.getControl();
				if (defineWidget != null && defineWidget.isDirty()) {
					return true;
				}
			}
		}
	...
	public void dispose() {
		if (getEditorInput() instanceof DefineScenesShotInsertEditorInput) {
			DefineScenesShotInsertEditorInput input = (DefineScenesShotInsertEditorInput) getEditorInput();
	...
	protected void createPartControlImpl(final Composite parent) {
		...
		tabFolderTop
				.setSelectionBackground(Colors.VIDEO_OVERVIEW_ITEM_BG_SELECTED
						.getColor());
		...
		tabFolder.setSelectionBackground(Colors.VIDEO_OVERVIEW_ITEM_BG_SELECTED
				.getColor());
	...
	public void createNewScene() {
		...
		SceneDefineWidget sdw = new SceneDefineWidget(tabFolder, SWT.CENTER,
	...
	private void openTab(Scene scene) {
		...
		if (!isOpen(scene)) {
			if (tabFolder != null) {
				...
				SceneDefineWidget sdw = new SceneDefineWidget(tabFolder,
	...
	public void propertyChange(PropertyChangeEvent evt) {
		...
		if (evt.getPropertyName().equals(Scene.PROP_END)) {
			lastEndTime = (Long) evt.getNewValue();
		}

		// Name des Medionobjekts hat sich geändert
		if (evt.getSource() instanceof Video && evt.getPropertyName() != null) {
