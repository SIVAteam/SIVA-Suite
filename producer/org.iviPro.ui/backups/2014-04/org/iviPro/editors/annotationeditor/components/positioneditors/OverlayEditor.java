...
public class OverlayEditor {
	...
	private int maxVidWidth = 650;
	private int maxVidHeight = 500;
	 ...
	 * @param pathItems
	 */
	public OverlayEditor(List<OverlayPathItem> pathItems, Object content) {
	...
	public OverlayEditor(NodeScene nodeScene, SivaTime startTime,
			SivaTime endTime, List<OverlayPathItem> pathItems, Object content, boolean isMark) {
		// erzeuge eine Shell für den Overlay Editor
		this.shell = new Shell(Display.getCurrent().getActiveShell(),
				SWT.DIALOG_TRIM | SWT.APPLICATION_MODAL);
	...
	public void open() {
		...
		videoWidth = Application.getCurrentProject().getSettings()
				.getAreaCenterWidth();
		videoHeight = Application.getCurrentProject().getSettings()
				.getAreaCenterHeight();
		...
		if (content != null) {
			...
			if(content instanceof Scene || content instanceof Video || content instanceof Picture || content instanceof String){
	...
	private void createToolbar() {
		...
		if (content != null) {
			if (content instanceof Scene || content instanceof Video
				...
				final ToolItem aspectSwitch = new ToolItem(bar, SWT.CHECK);
		...
		if (scale != null) {
			...
			scale.getSlider().addMouseListener(new MouseListener() {
				...
				public void mouseDown(MouseEvent me) {
					// Media player time has already been updated by listener in AbstractSivaSlider
					lastRecord = figMap.floorKey(mp.getMediaTime().getNano());
					...
					for (TimeFigure tfig : figMap.values()) {
						if (tfig.getMarkPosition().contains(me.x, me.y)) {
							// Adjust lastRecord when selecting a TimeFigure
	...
	private void createPositionandSizeLabel(){
		...
		GridData gd = new GridData(85,20);
		GridData gd2 = new GridData(75,20);
			posLabelx = new Label(c, SWT.NONE);
			posLabelx.setLayoutData(gd);
			posLabely = new Label(c, SWT.NONE);
			posLabely.setLayoutData(gd);
			posLabelh = new Label(c, SWT.NONE);
			posLabelh.setLayoutData(gd2);
			posLabelw = new Label(c, SWT.NONE);
			posLabelw.setLayoutData(gd2);
		updateOverlayBounds();
	}
	...
	private void updateOverlayBounds() {
		...
		posLabelx.setText(Messages.OverlayPathEditor_PositionLabel_x + loc.x);
		posLabely.setText(Messages.OverlayPathEditor_PositionLabel_y + loc.y);
		posLabelh.setText(Messages.OverlayPathEditor_PositionLabel_Height + loc.height);
		posLabelw.setText(Messages.OverlayPathEditor_PositionLabel_Width + loc.width);
