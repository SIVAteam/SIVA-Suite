...
public class PathHelper {
	...
	/**
	 * Gibt den Pfad zur ffmpeg.exe zurueck
	 * 
	 * @return
	 */
	public static File getPathToFFMpegExe() {
		String libDir = "."; //$NON-NLS-1$
		String[] args = Platform.getApplicationArgs();
		for (int i = 0; i < (args.length - 1); i++) {
			if (args[i].equals("-libdir")) { //$NON-NLS-1$
				libDir = args[i + 1];
			}
		}
	 ...
	 * @return
	 */
	public static File getPathToFFMpegExportExe() {
		String libDir = "."; //$NON-NLS-1$
		String[] args = Platform.getApplicationArgs();
		for (int i = 0; i < (args.length - 1); i++) {
			if (args[i].equals("-libdir")) { //$NON-NLS-1$
		...
		return new File(libDir + File.separatorChar + "ffmpeg_export.exe"); //$NON-NLS-1$
	}
	...
	/**
	 * Gibt den Pfad zu den Player-Dateien zurueck, die beim Export kopiert
	 * werden muessen.
	 * 
	 * @return
	 */
	public static File getPathToPlayerFiles(ExportType type) {
		String libDir = "."; //$NON-NLS-1$
		String[] args = Platform.getApplicationArgs();
		for (int i = 0; i < (args.length - 1); i++) {
			if (args[i].equals("-libdir")) { //$NON-NLS-1$
		...
		// kopiere die Dateien des HTML5 Player
		if (type == ExportType.HTML5) {
			return new File(libDir + File.separatorChar + "HTML5player"
					+ File.separator);

		} else if (type == ExportType.SMIL) {
			return new File(libDir + File.separatorChar + "smil"
					+ File.separator);
		}
		return new File(libDir + File.separatorChar + "player" + File.separator);//$NON-NLS-1$
	}

	/**
	 * Gibt Pfad zur FOBS DLL zurueck
	 * 
	 * @return
	 */
	public static File getPathToFOBSDLL() {
		String libDir = "."; //$NON-NLS-1$
		String[] args = Platform.getApplicationArgs();
		for (int i = 0; i < (args.length - 1); i++) {
			if (args[i].equals("-libdir")) { //$NON-NLS-1$
		...
		return new File(libDir + File.separatorChar + "fobs4jmf.dll"); //$NON-NLS-1$
	}
