...
import mpeg7.SemanticBaseType.MediaOccurrence;
...
import org.iviPro.actions.nondestructive.SceneDetectionOptionEditorAction;
import org.iviPro.actions.undoable.FolderRenameAction;
...
import org.iviPro.actions.undoable.NewFolderAction;
	...
	protected void onDrop(DropTargetEvent event) {
		...
		if (objects != null && objects.length > 0) {
			if (objects[0] instanceof IAbstractBean) {
				...
				if (targetFolder.getData() instanceof MediaTreeGroup) {
					...
					while (!(sourceTest == groupAudio
							|| sourceTest == groupPicture
							|| sourceTest == groupSubtitles
							|| sourceTest == groupText || sourceTest == groupVideo)) {
						sourceTest = sourceTest.getParent();
					}
					while (!(targetTest == groupAudio
							|| targetTest == groupPicture
							|| targetTest == groupSubtitles
							|| targetTest == groupText || targetTest == groupVideo)) {
