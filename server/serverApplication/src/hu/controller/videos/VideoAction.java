 ...
 * {@link Video} as well as the ability to start or stop it. Furthermore it
 * provides the functionality to participate in a {@link Video}.
 */
@ManagedBean
@RequestScoped
public class VideoAction extends AController {
    ...
    private static final String MSG_SELF_DEFINED = "selfDefined";
    private static final String MSG_FULL_HD = "fullHD";
    private static final String MSG_HIGH_DEFINITION = "highDefinition";
    private static final String MSG_STANDARD_DEFINITION = "standardDefinition";
    private static final String MSG_SMARTPHONE = "smartphone";
    private static final String MSG_TABLET = "tablet";
     ...
     *            to inject.
     */
    public void setVideoPublicationBean(VideoPublicationBean videoPublicationBean) {
    ...
    /**
     * Create a new {@link Video} in the database with the information provided
     * in the {@link VideoBean}.
     * 
	...
	if (sessionData.getUserId() != null && userStore.findById(sessionData.getUserId()) != null) {
	...
	if (sessionData.getUserId() == null
		|| (!(ownedGroups != null && ownedGroups.contains(group)) && userStore.getById(
			sessionData.getUserId()).getUserType() != EUserType.Administrator)) {
	...
	video.setParticipationRestriction(videoBean.getParticipationRestriction());
	...
	try {
	    video = videoStore.create(video, videoBean.getGroupId());
	} catch (InconsistencyException e) {
	    this.sendGlobalMessageToFacelet(this.getCommonMessage(MSG_CREATE_FAILED));
	    return "";
	}

	videoBean.setId(video.getId());

	this.sendMessageTo(CREATE_VIDEO_FORM, this.getCommonMessage(MSG_CREATE_SUCCESS));
	return EDIT_VIDEO_FACELET + APPEND_GET_PARAMETER + APPEND_VIDEO_ID + video.getId();
    }
     ...
     * 
     * Only {@link User}s of {@link EUserType#Administrator} and
     * {@link EUserType#Tutor}, who are owners of the {@link Video}, have
     * permission to use.
     * 
     * @return the next page to show as {@link String}.
     * 
	...
	Group group = video == null ? null : groupStore.getForVideo(video);

	/*
	 * Redirect, if there is no video with the given id or the user is not
	...
	this.videoBean
		.setPasswordAvailable(video.getParticipationRestriction() == EParticipationRestriction.Password);
	this.videoBean
		.setTokenAvailable(video.getParticipationRestriction() == EParticipationRestriction.Token);

	// Prepopulate the restrictions
	this.videoBean.setParticipationRestriction(video.getParticipationRestriction());

	this.videoBean.setVideoAvailable((new File(VIDEO_DESTINATION + "/" + video.getDirectory()
		+ "/video.zip")).exists());
	...
	if (!cxt.isPostback()) {
	    ...
	    if (this.videoBean.getParticipationRestriction().equals(
		    EParticipationRestriction.Password)) {
		video.setPassword(this.videoBean.getPassword());
	    }

	    if (this.videoBean.getStart() != null && this.videoBean.getVideo() == null
		    && !this.videoBean.isVideoAvailable()) {
		this.sendGlobalMessageToFacelet(this.getCommonMessage(MSG_NO_VIDEO_ARCHIVE));
		return null;
	    }
	    ...
	    // Move uploaded video if there has been uploaded one or if it's a
	    // unit test
		...
		videoBean.setId(video.getId());
		videoStore.moveVideo(video.getId(), -1, videoBean.getGroupId());
		...
		this.sendMessageTo(LIST_VIDEO_TABLE, this.getCommonMessage(MSG_EDIT_SUCCESS));
	    } catch (InconsistencyException e) {
	...
	if (!cxt.isPostback()) {
	    ...
	    this.videoBean.setZipDownloadEnabled(video.isZipDownloadEnabled());
	    this.videoBean.setChromeAppURL(video.getChromeAppURL());
	    this.videoBean.setSize(video.getSize());
	    this.videoBean.setLastUpdated(video.getLastUpdated());
	} else if (cxt.isPostback() && !cxt.isValidationFailed()) {
	...
	Video video = videoStore.findById(videoBean.getId());
	// System.out.println(videoBean.getScreenResolution());
	// System.out.println(this.getCommonMessage(MSG_SELF_DEFINED));
	// System.out.println(videoBean.getScreenResolution().equalsIgnoreCase(this.getCommonMessage("resolution_SelfDefined")));
	// if(videoBean.getScreenResolution().equalsIgnoreCase(this.getCommonMessage(MSG_SELF_DEFINED))){
	// videoBean.setSelfDefinedScreenResolution(true);
	// }

	Settings settings = new Settings();
	int width;
	int height;
	switch (videoBean.getScreenResolution()) {
	case MSG_SELF_DEFINED:
	    width=Integer.parseInt(videoBean.getWidth());
	    height= Integer.parseInt(videoBean.getHeight());
	    break;
	case MSG_SMARTPHONE:
	    width=800;
	    height=480;
	...
	case MSG_FULL_HD:
	    ...
	    height=1080;
	    break;
	case MSG_STANDARD_DEFINITION:
	    width=1024;
	    height=576;
	    break;
	case MSG_HIGH_DEFINITION:
	    width=1280;
	    height=720;
	    break;
	case MSG_TABLET:
	...
	if (videoBean.getId() != null) {
	    ...
	    try {
		videoId = Integer.parseInt((String) videoBean.getHiddenValue());
	    } catch (NumberFormatException ignored) {
	    }
	}
	videoBean.setId(videoId);
	videoBean.setHiddenValue(videoId);
	return null;
    }
	...
	IVideoStore qqs = this.persistenceProvider.getVideoStore();
	...
	Video video = qqs.findById(this.videoBean.getId());
	if (video == null) {
	    this.redirectTo(RESTRICTION_ERROR_FACELET);
	    return null;
	}
	...
	if (this.getCurrentFcInstance().isPostback()) {
	    ...
	    this.sendMessageTo(LIST_VIDEO_TABLE, this.getCommonMessage(MSG_VIDEO_DELETED_SUCCESS));
    ...
    /**
     * Start a {@link Video} independently from a previous set starting time.
     * 
     * @return the next page to show as {@link String}.
     * 
     * @throws IOException
	...
	if (!fcxt.isPostback() && !fcxt.isValidationFailed()) {
		...
		this.sendMessageTo(LIST_VIDEO_TABLE,
			this.getCommonMessage(MSG_VIDEO_STARTED_SUCCESS));

	    } catch (InconsistencyException e) {
		// redirect to list video facelet in all cases
		this.redirectTo(LIST_VIDEO_FACELET);

		// video has to contain a password if it has appropriate publish
		// function
    ...
    /**
     * Stop a {@link Video} independently from a previous set ending time.
     * 
     * Only {@link User}s of {@link EUserType#Administrator} and
     * {@link EUserType#Tutor}, who are owners of the {@link Video}, have
     * permission to use.
	...
	if (currentUser.getUserType() != EUserType.Administrator
		&& !userStore.isUserOwnerOfVideo(currentUser.getId(), this.videoBean.getId())) {
	    this.sendGlobalMessageToFacelet(this.getCommonMessage(MSG_NO_PERMISSION));
	    this.redirectTo(RESTRICTION_ERROR_FACELET);
	    return null;
	}
	...
	if (!fcxt.isPostback() && !fcxt.isValidationFailed()) {
		...
		this.sendMessageTo(LIST_VIDEO_TABLE,
			this.getCommonMessage(MSG_VIDEO_STOPPED_SUCCESS));
	    } catch (InconsistencyException e) {

		this.sendGlobalMessageToFacelet(this.getCommonMessage(MSG_VIDEO_STOPPED_FAILURE));
	    }
	}
	return LIST_VIDEO_FACELET;
    }
    ...
    /**
     * Participate in a {@link Video}. Possible restrictions for a {@link Video}
     * are defined in {@link EParticipationRestriction}
     * 
     * @return the next page to show as {@link String}.
     * 
     */
	...
	FacesContext fcxt = this.getCurrentFcInstance();
	IVideoStore qStore = this.persistenceProvider.getVideoStore();
	...
	if (this.sessionData.getUserId() != null) {
	    ...
	    if (user.getUserType().equals(EUserType.Administrator)
		    || uStore.isUserOwnerOfVideo(user.getId(), video.getId())) {
		this.redirectTo(WATCH_VIDEO_FACELET + APPEND_GET_PARAMETER + APPEND_VIDEO_ID
			+ video.getId());
		return null;
	    }
	}
	...
	if (!fcxt.isPostback()
		...
		&& (video.getParticipationRestriction() == EParticipationRestriction.Password || video
			.getParticipationRestriction() == EParticipationRestriction.Token)) {
	    return null;
	}

	// verify that video has started and has not stopped yet
	Date currentDate = new Date();
	if (video.getStart() == null || video.getStart().compareTo(currentDate) > 0) {

	    // video has not yet started
	    this.sendGlobalMessageToFacelet(String.format(
		    this.getCommonMessage(MSG_VIDEO_NOT_YET_STARTED), video.getTitle())
		    + " "
		    + (video.getStart() == null ? "" : String.format(
			    this.getCommonMessage(MSG_VIDEO_START_AT), video.getStart())));
	    return LIST_VIDEO_FACELET + APPEND_GET_PARAMETER;
	} else if (video.getStop() != null && video.getStop().compareTo(currentDate) < 0) {

	    // video has stopped
	    this.sendGlobalMessageToFacelet(String.format(
		    this.getCommonMessage(MSG_VIDEO_ALREADY_STOPPED), video.getTitle()));
	    return LIST_VIDEO_FACELET + APPEND_GET_PARAMETER;
	}
	...
	if (video.getParticipationRestriction() == EParticipationRestriction.Password) {
	    ...
	    if (video.getPassword() == null || !video.getPassword().equals(videoBean.getPassword())) {

		this.sendMessageTo(PASSWORD_FIELD,
			this.getCommonMessage(MSG_VIDEO_PASSWORD_NOT_CORRECT));
	    } else {
		...
		if (token == null) {
		    this.sendMessageTo(TOKEN_FIELD, this.getCommonMessage(MSG_TOKEN_NOT_FOUND));
		} else if (token != null && !token.isForParticipation()) {

		    // check if token is valid for participating in Video
		    this.sendMessageTo(TOKEN_FIELD,
			    this.getCommonMessage(MSG_TOKEN_NOT_VALID_PARTICIPATION));
		    this.redirectTo(ACCESS_VIDEO_FACELET);
		} else {

		    // check if valid token belongs to specified video and
		    // redirect if so
		    Video tokenVideo = qStore.findByToken(token.getToken());
		    if (!tokenVideo.getId().equals(video.getId())) {
			this.sendMessageTo(TOKEN_FIELD,
				this.getCommonMessage(MSG_TOKEN_NOT_VALID_VIDEO));
		    } else {

			// redirect to video if everything is okay
			this.redirectTo(WATCH_VIDEO_FACELET + APPEND_GET_PARAMETER
				+ APPEND_VIDEO_ID + video.getId() + "&token=" + token.getToken());
		    }
		}
	    } else {
		this.sendMessageTo(TOKEN_FIELD, this.getCommonMessage(MSG_TOKEN_NOT_FOUND));
	...
	// verify that video has started and has not stopped yet
	if (user == null
		|| (!user.getUserType().equals(EUserType.Administrator) && !userStore
	 ...
	 * entry.setAdditionalInformation
	 * (((HttpServletRequest)this.getCurrentFcInstance
	 * ().getExternalContext().getRequest()).getRemoteAddr());
	 * entry.setTime(new Date()); entries.add(entry);
	 */
	try {
	    this.persistenceProvider.getApiStore().createSivaPlayerLogEntries(entries);
	} catch (InconsistencyException ignore) {
	...
	try {
	    ...
	    fc.getExternalContext().redirect(
		    CommonUtils.buildContextPath("/sivaPlayerVideos/" + video.getDirectory()
			    + "/video.zip", "token=" + session.getSessionToken()));
	...
	if (user != null) {
	    if (restriction == EParticipationRestriction.Registered
	...
	if (restriction == EParticipationRestriction.Public) {
	    ...
	    // Check if encrypted video password is equal to the password in the
	    // URL
    ...
    /**
     * Save uploaded video archive to video directory. Extract archive in video
     * directory and keep copy of the archive. Create video directory if not
     * exist.
     * 
     * @param video
     *            contains the video
     * @return null if the video was successfully saved, an error message
     *         otherwise.
     */
	...
	// Check if SIVA directory and index.html exist if zip download is
	// enabled
	if (this.videoBean.isZipDownloadEnabled()
		&& (!sivaDirectory.exists() || !indexFile.exists())) {
	...
	// and directories from to root video directory if so to put video
	// structure
	...
	if (directories.length == 1) {
		...
		for (String subFile : subFiles) {
		    (new File(destination.getAbsolutePath() + "/" + directory + "/" + subFile))
			    .renameTo(new File(destination.getAbsolutePath() + "/" + subFile));
		}
		try {
    ...
    /**
     * Set video file for unit tests as it's not possible to read a file and
     * convert it to a {@link Part} object.
     * 
     * @param videoFile
     *            to upload.
     */
    ...
    /**
     * Return the URL for embedding the video. 
     * @return the URL.
     */
    public String getEmbeddURL(){
	String url = "";
	try {
	    url = CommonUtils.buildContextPath("/sivaPlayerVideos/" + this.videoBean.getDirectory() + "/" + "watch.html",
		    null);
    ...
    /**
     * Return the secret for embedding the video. 
     * @return the URL.
     */
    public String getEmbeddSecret(){
	return SecurityUtils.hash(this.videoBean.getParticipationRestriction().toString().toLowerCase() + this.videoBean.getDirectory()).substring(10, 40);
    }
}